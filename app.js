...
// KOMPLETNA VERZIJA SA PWA PODRŠKOM - POPRAVLJEN PROBLEM SA UČITAVANJEM
// DODATA FUNKCIJA ZA UPDATE POGREŠNIH TEMPERATURA I PRENOS ZALIHA
// POPRAVLJENO RAČUNANJE PRVOG DANA SEZONE
...
class PeletTracker {
    constructor() {
        ...
        this.seasonLastView = {};
        this.isLoading = false; // ZAŠTITA OD DUPLOG UČITAVANJA
    ...
    setupEventListeners() {
        ...
        // NOVO DUGME ZA UPDATE POGREŠNIH TEMPERATURA
    ...
    addSampleData() {
        ...
        // POPRAVLJENO: Uključuje i prvi dan
    ...
    // ========== MODIFIKOVANA METODA ZA DODAVANJE SEZONE ==========
    async dodajSezonu() {
        const seasonData = {
        ...
        // PITANJE ZA PRENOS ZALIHA AKO POSTOJI PRETHODNA SEZONA
        let prenesiZalihe = false;
        let prethodnaSezonaId = null;
        
        if (Object.keys(this.state.seasons).length > 0) {
            // Pronađi poslednju sezonu
            const sezone = Object.values(this.state.seasons);
            sezone.sort((a, b) => new Date(b.period.end) - new Date(a.period.end));
            
            if (sezone.length > 0) {
                prethodnaSezonaId = sezone[0].id;
                prenesiZalihe = confirm(`Želite li da prenesete nepotrošeni pelet iz sezone "${prethodnaSezonaId}" u novu sezonu?`);
            }
        }
        ...
        // PRENOS ZALIHA AKO JE KORISNIK ODOBRIO
        if (prenesiZalihe && prethodnaSezonaId) {
            const prethodnaSezona = this.state.seasons[prethodnaSezonaId];
            if (prethodnaSezona) {
                const potrosnjaPrethodne = Object.values(prethodnaSezona.days).reduce(
                    (sum, dan) => sum + (dan.potrosnja || 0), 0
                );
                const preostalo = prethodnaSezona.pocetnaKolicina - potrosnjaPrethodne;
                
                if (preostalo > 0) {
                    novaSezona.pocetnaKolicina += preostalo;
                    UIHelpers.showAlert(`Preneto ${preostalo.toFixed(2)} džakova iz prethodne sezone`, 'success');
                }
            }
        }
        
        // POPRAVLJENO: Pravilno uključuje prvi dan
        const start = new Date(seasonData.pocetniDatum);
        const end = new Date(seasonData.krajDatum);
        
        // Resetujemo vreme na početak dana da bismo izbegli probleme sa vremenskom zonom
        start.setHours(0, 0, 0, 0);
        end.setHours(0, 0, 0, 0);
        
        console.log(`Kreiram sezonu od ${start.toISOString()} do ${end.toISOString()}`);
        
        // POPRAVLJENO: Koristimo <= da uključimo i poslednji dan
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
            const dateStr = d.toISOString().split('T')[0];
        ...
        console.log(`Kreirano ${Object.keys(novaSezona.days).length} dana`);
        ...
        UIHelpers.showAlert(`Sezona "${seasonData.id}" je uspešno kreirana sa ${Object.keys(novaSezona.days).length} dana!`, 'success');
        
        setTimeout(() => {
            if (confirm(`Želite li da preuzmete temperature za ovu sezonu?`)) {
                this.dodajTemperatureZaSezonu();
            }
        }, 1000);
    }

    // ========== NOVA METODA ZA PRENOS ZALIHA ==========
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select id="izvornaSezonaSelect" class="form-input">
                            ${sezone.map(s => `<option value="${s.id}">${s.id} (${this.izracunajPreostalo(s)} džakova)</option>`).join('')}
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <small class="form-help">Ako ostavite prazno, preneće se sav preostali pelet</small>
                    </div>
                </div>
    ...
    izracunajPreostalo(sezona) {
        const potrosnja = Object.values(sezona.days).reduce((sum, dan) => sum + (dan.potrosnja || 0), 0);
        return (sezona.pocetnaKolicina - potrosnja).toFixed(2);
    }
    ...
    async izvrsiPrenosZaliha(izvornaId, ciljnaId, kolicina = null) {
        ...
        // Izračunaj preostalo u izvornoj sezoni
        const potrosnjaIzvorne = Object.values(izvorna.days).reduce((sum, dan) => sum + (dan.potrosnja || 0), 0);
        const preostaloUIzvornoj = izvorna.pocetnaKolicina - potrosnjaIzvorne;
        ...
        // Smanji početnu količinu izvorne sezone (da se ne bi duplo računalo)
        izvorna.pocetnaKolicina -= kolicinaZaPrenos;
        
        // Povećaj početnu količinu ciljne sezone
        ciljna.pocetnaKolicina += kolicinaZaPrenos;
        ...
        // Osveži prikaz ako je aktivna neka od sezona
        ...
        UIHelpers.showAlert(`Uspešno preneto ${kolicinaZaPrenos.toFixed(2)} džakova`, 'success');
    }
    ...
    // ========== POPRAVLJENA METODA ZA UČITAVANJE SEZONE ==========
    async ucitajSezonu() {
        ...
        // Ako ista sezona, samo osveži prikaz
    ...
    renderMonthlyCalendar() {
        ...
        // POPRAVLJENO: Pravilno računanje prvog dana u nedelji
        let firstDayOfWeek = firstDay.getDay();
        // Ako je nedelja (0), pomeramo na 6, inače smanjujemo za 1 (ponedeljak je 1)
        ...
        for (let day = 1; day <= lastDay.getDate(); day++) {
            ...
            // POPRAVLJENO: Pravilno poredjenje datuma (uključuje i prvi i poslednji dan)
    ...
    // ========== ORIGINALNA METODA ZA DODAVANJE TEMPERATURA ==========
    ...
    // ========== NOVA METODA ZA UPDATE POGREŠNIH TEMPERATURA ==========
    async updatePogresneTemperature() {
        ...
        // Proveri koliko dana ima temperature
        ...
        try {
            // Sačuvaj stare temperature za slučaj povratka (opciono)
            ...
            // Privremeno sačuvaj u localStorage za slučaj nužde
            localStorage.setItem('temp_backup_' + season.id, JSON.stringify(stareTemperatures));
            
            // Resetuj sve temperature na 0 (da bi ih updateSeasonTemperatures preuzeo)
            Object.keys(season.days).forEach(datum => {
                season.days[datum].temperatura = 0;
            });
            
            // Preuzmi nove temperature
            const azuriranaSezona = await temperatureService.updateSeasonTemperatures(season);
            
            // Sačuvaj u data manager
            dataManager.updateSeason(season.id, azuriranaSezona);
            ...
            // Pokušaj da vrati stare temperature ako je moguće
    ...
    async renderRezime() {
        ...
        // POPRAVLJENO: Pravilno računanje broja dana u sezoni (uključuje i prvi i poslednji)
        ...
        container.innerHTML = `
            ...
            <div class="info-banner" style="margin-top: 10px; background: #e8f4f8;">
        ...
        seasons.forEach(season => {
            ...
            html += `
                <tr ${isActive ? 'class="active-season"' : ''}>
                    ...
                    <td>${UIHelpers.formatNumber(prosecnaDnevna, 2)}</td>
                    <td>${UIHelpers.formatNumber(prosecnaTemp, 1)}</td>
                    <td>${UIHelpers.formatNumber(preostalo, 1)}</td>
